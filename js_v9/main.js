...
function showHelp() {
    $("#inst_main_body").html("不要让积累的色块超过六角形的灰色边框<br><br>" + (settings.platform != 'mobile' ? '按向左或向右方向键' : '触动屏幕的左边或右边') + "  来旋转六角形<br><br>3个相同色块连在一起就可以被消除和得分<br><br>" + (settings.platform == 'mobile' ? '触动里面的六角形可以加速游戏' : "按住向下键可以加速两倍"));
    if (gameState == 1) {
        pause();
    }
